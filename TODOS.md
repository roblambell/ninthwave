...
**Test plan:**
- Unit test: verify `mark-done` is called when an item transitions to `done`
- Integration test: after orchestrator run, TODOS.md on disk has the item removed
- Edge case: multiple items reaching `done` in the same tick — all marked
...
**Test plan:**
- Unit test: verify cleanup sweep is called before `orchestrate_complete` is emitted
- Edge case: worktree for an item that was never launched (queued → stuck path)
- Edge case: worktree removal fails (locked file) — should log warning, not crash
...
**Test plan:**
- Unit test: verify `orchestrate_complete` JSON includes `items` array
- Each item object has `id`, `state`, and `prUrl` fields
- Edge case: item with no PR (stuck before PR opened) — `prUrl` should be null
