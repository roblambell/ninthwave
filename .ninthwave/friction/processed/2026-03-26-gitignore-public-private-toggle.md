# .ninthwave gitignore: public vs private repos and todo visibility (2026-03-26)

## What happened
Setting up ninthwave on strait (a public repo). `ninthwave init`/`setup` commits the entire `.ninthwave/` directory with no option to ignore parts of it. This works fine for a private org repo where todos are team assets, but on a public repo by a solo dev, committing the task backlog is unnecessary noise in the git history and visible to anyone browsing the repo.
