...
function makeWorkItemsDir(): string {
  const base = join(
    require("os").tmpdir(),
...
function writeWorkItemFile(workDir: string, id: string, priority = "medium", domain = "testing"): void {
...
describe("cmdMarkDone", () => {
  it("deletes a single work item file", () => {
    const workDir = makeWorkItemsDir();
    writeWorkItemFile(workDir, "M-CI-1");
    writeWorkItemFile(workDir, "H-CI-2");
  ...
  it("deletes multiple work item files at once", () => {
    const workDir = makeWorkItemsDir();
    writeWorkItemFile(workDir, "M-CI-1");
    writeWorkItemFile(workDir, "H-CI-2");
    writeWorkItemFile(workDir, "C-UO-1");
